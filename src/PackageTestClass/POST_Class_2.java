...
public class POST_Class_2 {
	...
	public static void execute() throws IOException {
		for (int i=0; i<5; i++)
		{				
			int statusCode=APICommonFunction.statusCode(ReqRepositoryPost.baseURI(), ReqRepositoryPost.resource(), ReqRepositoryPost.requestBody2());
		
		if (statusCode==201)
		{
			String responseBody =APICommonFunction.responseBody(ReqRepositoryPost.baseURI(), ReqRepositoryPost.resource(), ReqRepositoryPost.requestBody2());
			System.out.println(responseBody);
			
		POST_Class_2.validator(statusCode, responseBody);
		Utilitycommonfunction.evidencefilecreator("Sample2", ReqRepositoryPost.requestBody2(), responseBody);
		break;
		}
		...
		JsonPath jspreq=new JsonPath(ReqRepositoryPost.requestBody2());
