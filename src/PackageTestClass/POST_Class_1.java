...
public class POST_Class_1 {
	...
	public static void execute() throws IOException {
		...
		for (int i=0; i<5; i++)
		{				
			int statusCode=APICommonFunction.statusCode(ReqRepositoryPost.baseURI(), ReqRepositoryPost.resource(), ReqRepositoryPost.requestBody1());
		
		if (statusCode==201)
		{
			String responseBody =APICommonFunction.responseBody(ReqRepositoryPost.baseURI(), ReqRepositoryPost.resource(), ReqRepositoryPost.requestBody1());
			System.out.println(responseBody);
			
		POST_Class_1.validator(statusCode, responseBody);
		Utilitycommonfunction.evidencefilecreator("Sample1", ReqRepositoryPost.requestBody1(), responseBody);
		break;
		}
		...
		JsonPath jspreq=new JsonPath(ReqRepositoryPost.requestBody1());
