...
public class POST_Class_4 {
	...
	public static void execute() throws IOException {
		...
		for (int i=0; i<5; i++)
		{				
			int statusCode=APICommonFunction.statusCode(ReqRepositoryPost.baseURI(), ReqRepositoryPost.resource(), ReqRepositoryPost.requestBody4());
		
		if (statusCode==201)
		{
			String responseBody =APICommonFunction.responseBody(ReqRepositoryPost.baseURI(), ReqRepositoryPost.resource(), ReqRepositoryPost.requestBody4());
			System.out.println(responseBody);
			
		POST_Class_4.validator(statusCode, responseBody);
		Utilitycommonfunction.evidencefilecreator("Sample4", ReqRepositoryPost.requestBody4(), responseBody);
		break;
		}
		...
		JsonPath jspreq=new JsonPath(ReqRepositoryPost.requestBody4());
